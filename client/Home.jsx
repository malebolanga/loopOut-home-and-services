...
const PROPERTY_TYPES = {
  rent: { icon: <FaKey />, label: "Rent" },
  sale: { icon: <FaTag />, label: "Sale" },
  land: { icon: <FaLandmark />, label: "Land" },
  over: { icon: <FaBuilding />, label: "Commercial" },
  office: { icon: <FaCity />, label: "Office" },
};

const SERVICE_TYPES = {
  cleaning: { icon: <FaBroom />, label: "Cleaning" },
  maintenance: { icon: <FaTools />, label: "Maintenance" },
  moving: { icon: <FaTruckMoving />, label: "Moving" },
  landscaping: { icon: <FaTree />, label: "Landscaping" },
  catering: { icon: <FaUtensils />, label: "Catering" },
  other: { icon: <FaQuestionCircle />, label: "Other" },
  // New service types
  daycare: { icon: <FaChild />, label: "DayCare" },
  schoolTransport: { icon: <FaBus />, label: "School" }
};

// Updated helper types with Chef category
const HELPER_TYPES = {
  domestic: { icon: <FaUserTie />, label: "General Help" },
  errand: { icon: <FaShoppingBag />, label: "Errand Runner" },
  tutor: { icon: <FaBookReader />, label: "Tutor" },
...
const LOCAL_EVENT_TYPES = {
  ...
  sports: { icon: <FaFutbol />, label: "Sports" },
  art: { icon: <FaPalette />, label: "Art & Culture" },
  community: { icon: <FaUsers />, label: "Community" },
  food: { icon: <FaUtensils />, label: "Food & Drink" },
  other: { icon: <FaQuestionCircle />, label: "Other Events" },
};
...
const TabButton = ({ activeTab, tabName, onClick, Icon, rotation, id }) => {
  return (
    <button
      className={`flex-1 min-w-0 flex flex-col items-center px-4 md:px-6 py-3 font-semibold transition-all duration-200 relative group ${activeTab === tabName
          ? "text-blue-600"
    ...
    >
      <div
        className={` 
          mb-1 p-2 rounded-full transition-all duration-200 transform
          ${activeTab === tabName
            ? "bg-blue-100"
      ...
      >
        {/* 3D Icon Container */}
        <div className="perspective-container" style={{ perspective: '100px' }}>
          <div
            className="transform-3d transition-transform duration-100 ease-out"
            style={{
      ...
      <div
        className={` 
          absolute bottom-0 h-0.5 w-full bg-blue-600 transition-all duration-300 ease-out
...
export default function Home() {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      const calculateRotation = (elementId, sensitivity) => {
        ...
        const distX = (e.clientX - elementCenterX) / (elementRect.width / 2);
        const distY = (e.clientY - elementCenterY) / (elementRect.height / 2);
  ...
  if (activeTab === "properties") {
    currentSelectedType = propertyType === "all"
      ? { label: "Property Type", icon: <FaHome /> }
      : PROPERTY_TYPES[propertyType] || { label: "Property Type", icon: <FaHome /> };
  } else if (activeTab === "services") {
    currentSelectedType = serviceType === "all"
      ? { label: "Service Type", icon: <FaTools /> }
      : SERVICE_TYPES[serviceType] || { label: "Service Type", icon: <FaTools /> };
  } else if (activeTab === "helper") {
    currentSelectedType = helperType === "all"
      ? { label: "Helper Type", icon: <FaUserAlt /> }
      : HELPER_TYPES[helperType] || { label: "Helper Type", icon: <FaUserAlt /> };
  } else if (activeTab === "events") {
    currentSelectedType = eventType === "all"
      ? { label: "Event Type", icon: <FaCalendarAlt /> }
      : LOCAL_EVENT_TYPES[eventType] || { label: "Event Type", icon: <FaCalendarAlt /> };
  }
        ...
        >
          <div className="flex mb-6 w-full justify-around">
            ...
            <TabButton
              ...
              onClick={setActiveTab}
              Icon={FaTools}
              rotation={servicesRotation}
              id="services-icon"
            />

            {/* Personal Helpers Tab */}
            <TabButton
              activeTab={activeTab}
              tabName="helper"
              onClick={setActiveTab}
...
<div className="mt-0">
  <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6 text-center">
    {activeTab === "all" && "Everything Nearby"}
    {activeTab === "properties" && "All Property Listings"}
    {activeTab === "services" && "Professional Service Offerings"}
    {activeTab === "helper" && "Qualified Personal Helpers"}
    {activeTab === "events" && "Upcoming Local Events"}
  </h2>
  ...
  {activeTab !== "all" && filteredListingsFull.length > visibleListings && !isLoading && (
    <div className="flex justify-center mt-8" ref={scrollRef}>
      <button
        onClick={() => setVisibleListings((prev) => prev + LOAD_MORE_COUNT)}
        className="px-6 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      >
        Load More
      </button>
    </div>
