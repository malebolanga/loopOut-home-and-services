...
import { Search, SlidersHorizontal, Map, List as ListIcon, MapPin } from 'lucide-react';
...
const EventsHomePage = () => {
  ...
  const filteredEvents = useMemo(() => {
    ...
    let categoryEvents = activeTab === 'all' ? events : events.filter(event => event.type === activeTab);
    if (searchQuery.trim()) {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="relative">
            <div className="w-12 h-12 border-4 border-gray-200 dark:border-gray-800 border-t-gray-900 rounded-full animate-spin mx-auto mb-4" />
          </div>
          <div className="text-lg font-semibold text-gray-900 dark:text-white">Finding upcoming events...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <div className="flex gap-4 justify-center">
            <button onClick={() => window.location.reload()} className="px-6 py-2.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors font-medium">
              Try Again
            </button>
            <button onClick={() => navigate('/')} className="px-6 py-2.5 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors font-medium">
              Go to Home
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="hidden md:flex items-center gap-3">
              <div className="flex items-center border border-gray-300 dark:border-gray-700 rounded-full px-1 py-1 pr-3 hover:shadow-md transition-shadow">
                ...
                <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-transparent text-sm font-semibold text-gray-700 dark:text-white focus:outline-none cursor-pointer">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-8 overflow-x-auto pt-2 pb-1 scrollbar-hide">
            {categories.map(category => (
              <button key={category.id} onClick={() => setActiveTab(category.id)} className={`flex flex-col items-center gap-2 min-w-[56px] pb-3 border-b-2 transition-colors duration-200 ${activeTab === category.id ? 'border-gray-900 text-gray-900 dark:text-white' : 'border-transparent text-gray-500 dark:text-white hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-gray-700'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-6 md:hidden">
          <div className="flex items-center border border-gray-300 dark:border-gray-700 rounded-full px-2 py-1 pr-3 shadow-sm">
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-transparent text-xs font-semibold text-gray-700 dark:text-white focus:outline-none cursor-pointer">
        ...
        {/* Map Toggle Button - AirBnB Red */}
        <button onClick={() => setShowMap(!showMap)} className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-airbnb-red text-white rounded-full px-5 py-3.5 flex items-center gap-2 shadow-xl hover:scale-105 transition-transform font-semibold text-sm">
          {showMap ? (
            <>
              <span>Show list</span>
          ...
          <div className={`${showMap ? 'lg:w-[60%] xl:w-[65%]' : 'w-full'}`}>
            {filteredEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <button onClick={() => { setActiveTab('all'); setSearchQuery(''); }} className="px-6 py-2.5 border border-gray-900 rounded-lg font-semibold text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  Remove all filters
                </button>
              </div>
            ) : (
