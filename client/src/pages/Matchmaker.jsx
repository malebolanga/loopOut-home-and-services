...
export default function Matchmaker() {
  ...
  const fetchDatabaseCards = async () => {
    ...
    try {
      const [listingsRes, servicesRes, helpersRes] = await Promise.all([
        fetch('/api/listing/get?limit=10'),
        fetch('/api/service/get?limit=10'),
        fetch('/api/helper/get?limit=10')
      ]);
  ...
  const handleBookingSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (cards.length > 0) {
        removeCard(cards[cards.length - 1].id, 'right');
      }
    }, 2000);
  };
    ...
    <div className="fixed inset-0 bg-gray-950 flex flex-col z-[200]">
      ...
      <div className="relative z-10 flex items-center justify-between p-6">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/10 rounded-full backdrop-blur-md">
          ...
          <span className="text-xs font-black text-white uppercase tracking-widest">Neural Matchmaker</span>
        </div>
        <div className="w-11" />
      </div>
