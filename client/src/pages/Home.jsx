...
function MobileAppHomepage({
  ...
  const currentTabLabel = useMemo(() => {
    if (activeTab === 'RecentAdded' || activeTab === 'Recent Added') return 'Recent Added';
    const found = tabs.find(t => t.id === activeTab);
    return found?.label || activeTab;
  }, [activeTab, tabs]);
        ...
        <section id="explore-section" className="mb-8">
          ...
          <div className="sticky top-0 z-40 bg-white/95 dark:bg-gray-950/95 backdrop-blur-2xl py-3 mb-4 -mx-4 px-4 border-b border-gray-100/80 dark:border-gray-800/80 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.05)] w-[calc(100%+2rem)]">
            <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide py-1 snap-x snap-mandatory">
              ...
              >
                <div className={`w-[46px] h-[46px] sm:w-[52px] sm:h-[52px] rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto transition-all duration-300 shadow-2xs ${showAllCategories
          ...
          {visibleCount < filteredItems.length ? (
            ...
            <div className="mt-8 flex flex-col items-center justify-center text-center py-3">
              <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-800 text-[10px] font-black text-slate-500">
                <Check className="w-3 h-3 text-emerald-500 stroke-[3]" />
                <span>You've viewed all {filteredItems.length} {currentTabLabel.toLowerCase()}</span>
              </div>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => navigate('/search')}
                className="mt-2.5 flex items-center gap-1.5 text-xs font-black text-rose-500 hover:text-rose-600 transition-colors cursor-pointer"
              >
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
        ...
        <div className="max-w-7xl mx-auto px-8 flex items-center justify-between">
          <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide py-1">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.92 }}
              whileHover={{ y: -2 }}
              className={`shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer focus:outline-none transition-all duration-200 border ${showAllCategories
            ...
            >
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${showAllCategories ? 'bg-white text-slate-700' : 'bg-white text-rose-600'
                } shadow-2xs`}>
                {showAllCategories ? (
      ...
      <main className="max-w-7xl mx-auto px-8 py-8">
        ...
        {isLoadingCurrentTab ? (
          ...
          <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-10">
              {filteredDesktopItems.slice(0, visibleCount).map((item, idx) => (
                <motion.div
                  key={item._id || idx}
            ...
            {visibleCount < filteredDesktopItems.length ? (
              ...
              <div className="mt-14 flex flex-col items-center justify-center text-center py-4">
                <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-800 text-[11px] font-black text-slate-500">
                  <Check className="w-3.5 h-3.5 text-emerald-500 stroke-[3]" />
                  <span>You've viewed all {filteredDesktopItems.length} {currentTabLabel.toLowerCase()}</span>
                </div>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => navigate('/search')}
                  className="mt-3 flex items-center gap-1.5 text-xs font-black text-rose-500 hover:text-rose-600 transition-colors cursor-pointer"
                >
                  <MagnifyingGlassIcon className="w-4 h-4" />
                  <span>Search all categories &amp; locations</span>
                </motion.button>
              </div>
            )}
          </div>
