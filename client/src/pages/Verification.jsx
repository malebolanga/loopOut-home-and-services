...
export default function Verification() {
  ...
  const PROTOCOLS = [
    { id: 'face', label: 'NEURAL FACE SCAN', icon: <FingerPrintIcon className="w-6 h-6" />, color: 'bg-rose-500', description: 'Advanced facial land-marking verification' },
    { id: 'doc', label: 'ID DEPLOYMENT', icon: <IdentificationIcon className="w-6 h-6" />, color: 'bg-indigo-500', description: 'Optical document scanning & authentication' },
  ...
  const handleVerifyFace = async () => {
    try {
      setIsProcessing(true);
      setError(null);
      // For this demo, we use a placeholder image URL, in real it would be from camera capture
      const res = await fetch('/api/verification/verify-face', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageUrl: currentUser.avatar }), 
      });
      const data = await res.json();
      if (data.success === false) {
        setError(data.message);
      ...
      setError('Face scan transmission failed.');
    } finally {
      setIsProcessing(false);
    }
  };
            ...
            >
                ...
                <div className="w-full max-w-2xl space-y-12">
                   <div className="text-center space-y-4">
                      <div className="px-5 py-2 bg-rose-600 text-white rounded-full inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em]">
                         <Scan size={14} /> LIVE SENSORS ACTIVE
                      </div>
                   ...
                   <div className="relative overflow-hidden rounded-[3rem] bg-gray-950 shadow-[0_50px_100px_-20px_rgba(225,29,72,0.3)] aspect-[4/3] flex items-center justify-center">
                     {cameraActive ? (
                       <>
                         ...
                         <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-[400px] border border-rose-500/50 rounded-[4rem] flex flex-col items-center">
                            <div className="absolute top-1/2 left-0 right-0 h-px bg-rose-500/30 animate-scan" />
                            <div className="absolute -top-10 left-1/2 -translate-x-1/2 text-[10px] font-black text-rose-500 uppercase tracking-widest">Target Alignment</div>
                         </div>
                       </>
                     ) : (
                       <button 
                      ...
                      {isProcessing && (
                         <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-12 text-center space-y-8">
                            ...
                            <div className="space-y-2">
                               <p className="text-rose-500 text-[10px] font-black uppercase tracking-[0.5em]">Neural Signal Analysis</p>
                               ...
                               <p className="text-white font-black text-3xl italic tracking-tighter">{progress}%</p>
                            </div>
                         </div>
                      )}
                   </div>
                   ...
                   {!isProcessing && cameraActive && (
                     <div className="flex justify-center gap-4">
                        ...
                        <button 
                          onClick={toggleCamera}
                          className="px-8 py-5 border border-gray-200 text-gray-400 hover:text-rose-600 rounded-[2rem] text-xs font-black uppercase tracking-widest transition-all"
                        >
                          Disable
                        </button>
                     </div>
              ...
              {method === 'doc' && (
                <div className="w-full max-w-xl text-center space-y-12">
                   <div className="space-y-4">
                      <h2 className="text-5xl font-black text-gray-950 tracking-tighter italic">ID SCANNER</h2>
