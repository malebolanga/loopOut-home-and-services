...
export default function MyListing() {
  ...
  // Filter out expired listings for each type and limit the count
  const activeListings = {
    rent: rentListings.filter((listing) => isListingActive(listing.createdAt)).slice(0, 1), // Limit to 4
    sale: saleListings.filter((listing) => isListingActive(listing.createdAt)).slice(0, 0), // Limit to 1
    offer: offerListings.filter((listing) => isListingActive(listing.createdAt)).slice(0, 0), // Limit to 1
    over: overListings.filter((listing) => isListingActive(listing.createdAt)).slice(0, 0), // Limit to 1
  };
