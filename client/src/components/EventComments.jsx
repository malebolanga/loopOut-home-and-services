...
const EventComments = ({ eventId, maxComments = 3, onTotalComments, onRatings, showSummary = false, cardStyle = false, externalRefreshTrigger = 0 }) => {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      const url = maxComments > 0 
        ? `/api/event-comments/${eventId}?limit=${maxComments}`
        : `/api/event-comments/${eventId}`;
  ...
  }, [eventId, maxComments, onTotalComments, onRatings]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        const booked = data.some(b => 
          (b.listing?._id === eventId || b.service?._id === eventId || b.helper?._id === eventId || b.event?._id === eventId) &&
  ...
  const handleLikeComment = async (commentId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/event-comments/like/${commentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${currentUser.token}`
  ...
  const handleSubmitReply = async (commentId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/event-comments/reply/${commentId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${currentUser.token}`
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/event-comments/${commentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${currentUser.token}`
        }
                ...
                <div className="flex items-center">
                  <div className="w-24 bg-gray-200 rounded-full h-2 mr-2">
                    <div 
                      ...
                      style={{ width: `${(ratings[name] / 5) * 100}%` }}
                    ></div>
                  </div>
                  <span className="font-medium">{ratings[name].toFixed(1)}</span>
                </div>
              </div>
            ))}
          </div>
