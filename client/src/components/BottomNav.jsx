...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] md:hidden">
      ...
      <div className="bg-white/95 backdrop-blur-xl border-t border-gray-200 px-6 py-2 pb-6 flex items-center justify-between shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`p-1 rounded-xl transition-all duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110'}`}>
                <Icon className={`w-6 h-6 ${isActive ? 'text-[#FF5A5F]' : 'text-gray-400'}`} />
              </div>
